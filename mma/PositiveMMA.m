...
StandardizeSumTerms2::usage="Method to standardize all terms in multiple sum expression (alternative to StandardizeSumTerms)";
...
(* Create shorthand of terms *)
ToShortHand::usage="Create symbolic shorthand of simple terms"
...
SolveEigenSystem::usage="Solve matrix eigenvalue problem. This method exists becuase MMA uses Roots before it strictly needs to."
...
TakeLargestExponent[Terms_,var_]:=Module[
	{Ans,MaxDeg,Terms2,kkkk,jjjj},
	(* Define holder for output *)
	Terms2 = Terms;
	(* Find largest exponent *)
	MaxDeg = Max@Table[ Exponent[Terms[[kkkk]],var], {kkkk,Length[Terms]} ];
	(* *)
	Terms2 = Select[Terms,MemberQ[#,var^MaxDeg] || (#==var^MaxDeg)&];
(*	Do[
		If[Exponent[Terms[[jjjj]][[kkkk]],var] \[NotEqual] MaxDeg,
			(*Remove the term*)
			Terms2[[jjjj]][[kkkk]]=0;
		];
		,
		{jjjj,Length[Terms]},{kkkk,Length[Terms[[jjjj]]]}
	];
	Print[Terms];
	(* Handle constant numerical entries in Terms *)
	Do[
		If[Length[Terms[[jjjj]]] \[Equal] 0,
			(*Remove the term*)
			Terms2[[jjjj]]=0;
		];
		,
...
(* Method to standardize all terms in multiple sum expression *)
Remove[StandardizeSumTerms2];
StandardizeSumTerms2[Terms_,var_]:=Module[ {Ans,CombineSumsRule1,CombineSumsRule2,StandardizeSumRule},
	
	(* Define rules to combine like sums *)
	CombineSumsRule1 = (u_ Sum[x_,z_]):>  Sum[Simplify[ u x],z];
	CombineSumsRule2 = ( Sum[x_,z_]+ Sum[y_,z_]):>  Sum[Simplify[  x+ y],z];
	StandardizeSumRule = { Sum[aa_ var^(p_+dex_),{dex_,dex0_,inf_}]:>Sum[(aa/.{dex->dex-p})var^dex,{dex,Max[dex0+p,0],inf}], aa_ Sum[bb_,cc_]:> Sum[Simplify[aa bb], cc] };
	Ans = ((Expand[Terms]//.CombineSumsRule1)//.StandardizeSumRule)(*//.CombineSumsRule2*);
	Return[Ans];
]
(* Given a sum of the form \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = 0\), \(\[Infinity]\)]\(
...
DeriveIndicialSolutions2[DyFunctions_,y_,x_,SingularPoints_,f_:f,RefDy_:None]:=Module[
	...
	V = Numerator[ Simplify[ Select[Apart@Expand@p[[1]],D[#,x]=!=0&] ] ];
	(* Solve indicial equation at evenery singular point *)
	SingularExponent = {};
	IndicialEquation = {};
	Do[
...
PythonForm[A_]:=Module[
	{Answer},
	(*  *)
	Answer = A/.{Exp[x_]:>exp[x]};
	(* Convert to string *)
	Answer = ToString[InputForm[Answer]];
	(* Handle Re[X] *)
	Answer = StringReplace[Answer,{"Re"->"real"}];
	(* Handle Im[X] *)
	Answer = StringReplace[Answer,{"Im"->"imag"}];
	(* Handle complex numbers *)
	Answer = StringReplace[Answer,{"^"->"**","I"->"1j"}];
	(* Handle Pi *)
	...
	(* Handle Conjugate[X] *)
	Answer = StringReplace[Answer,{"Conjugate"->"conj"}];
	(* Handle Sqrt[X] *)
	...
	(* Handle Log[X] *)
	Answer = StringReplace[Answer,{"Log"->"log"}];
	(* Handle Tanh[X] *)
...
TermSimplify[F_]:=Module[
	...
	(* Simplfy each Term *)
	Application = F[[0]];
	Ans = Application[ Table[ Simplify[F[[kkkk]]], {kkkk,Range[Length[F]]} ]/.{List->Sequence} ];
	Return[Ans];
]
(* Simplfy terms independently *)
...
TermFullSimplify[F_]:=Module[
	...
	(* Simplfy each Term *)
	Application = F[[0]];
	Ans = Application[ Table[ FullSimplify[F[[kkkk]]], {kkkk,Range[Length[F]]} ]/.{List->Sequence} ];
	Return[Ans];
]
...
Adjoint[Df_,f_,var_,assumptions_:{},NoConj_:False]:=Module[
	(* Internal variables *)
	{Answer,Coeffs,AdjCoeffs,ReducedDf,TemplateConjCoeffs,TemplateAdjDf,QList,AdjDf,zzz,kkkk,OrderD,Q},
	...
	TemplateConjCoeffs = Table[Symbol["QQQQ" <> ToString@kkkk][var], {kkkk, OrderD}];
...
Remove[ToShortHand]
ToShortHand[ Terms_, var_, label_:"A", StartDex_:0 ]:=Module[

	(* Internals *)
	{Answer,ConstantTerm,VariableTerms,ParamValues,ParamRules,
	A,B,B0,B1,C,VarPart,k,Params,ConstantParamValue,ShortHandTerms,
	VariableParamValues,ShortHandConstantTerm,ShortHandVariableTerms},
	
	(* Main *)
	A = Apart[Terms,var];
	Params = Table[ ToExpression[label<>ToString[k+StartDex-1]], {k,Range[Length[A]]} ];
	
	(*  *)
	ConstantTerm = Select[A,D[#,var]==0&];
	ConstantParamValue = { ConstantTerm };
	VariableTerms = Apart[Simplify[A - ConstantTerm],var];
	
	(*  *)
	ShortHandConstantTerm = { Params[[1]] };
	
	VariableParamValues = {};
	ShortHandVariableTerms = {};
	...
	Table[
		...
		If[
			...
			,
			C = Apart[B0,var];
			VarPart = Select[C,D[#,var]=!=0&];
			,
			C = Apart[B1,var];
			VarPart = 1/Select[C,D[#,var]=!=0&];
		];
		VariableParamValues = Join[VariableParamValues,{Simplify[B/VarPart]}];
		ShortHandVariableTerms = Join[ ShortHandVariableTerms, {Simplify[Params[[k+1]] VarPart]} ];
		
		,
		{k,Range[Length[VariableTerms]]}
		
	];
	

	(*  *)
	ParamValues = Join[ConstantParamValue,VariableParamValues];
	ShortHandTerms = Total[ Join[ShortHandConstantTerm,ShortHandVariableTerms] ];
	ParamRules = Table[ Params[[k]] -> ParamValues[[k]], {k,Range[Length[ParamValues]]} ];
	
	(* Return *)
	Return[{ShortHandTerms,Params,ParamRules}];
]


Remove[SolveEigenSystem];
SolveEigenSystem[X_]:=Module[
	(* Internals *)
	{Vals,Vecs,\[Lambda],kkkk,UseMMAFunction,TestValue},
	(* Main *)
	UseMMAFunction = Length[X]>3;
	If[ UseMMAFunction,
		(* Use MMA Function; else use more manual approach *)
		{Vals,Vecs} = Eigensystem[X];
	,
		(* Compute Eigenvalues *)
		Vals = \[Lambda]/.Solve[CharacteristicPolynomial[X,\[Lambda]]==0,\[Lambda]];
		(* Compute Eigenvectors *)
		Vecs = Transpose[Table[
			(**)
			\[Lambda] = Vals[[kkkk]];
			NullSpace[ X- \[Lambda] IdentityMatrix[Length[X]] ]
			,
			{kkkk,Range[Length[Vals]]}
		]][[1]];
	];
	(*Print[Vals[[1]]];
	TestValue = Simplify[(X * Vecs[[1]]) \[Equal] (Vals[[1]] * Vecs[[1]])];
	If[ TestValue\[Equal]True,
		Print["Good."];,
		Print["Not good."];
	];*)
	(* Return answer *)
